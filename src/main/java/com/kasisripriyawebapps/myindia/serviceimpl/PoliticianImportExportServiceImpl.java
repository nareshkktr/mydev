...
public class PoliticianImportExportServiceImpl implements PoliticianImportExportService {
			...
			for (WebElement col : cells) {
				if (processFromNextRow) {
					try {
						...
						processSarpanchas(findAnchor.getAttribute("href"), findAnchor.getText(),
								LocationConstants.STATE_ANDHRA_PRADESH);
	...
	private void parseSrikakulamSarpanchDetails(String fileName, String districtName, String stateName)
			throws IOException, InternalServerException {
		...
		if (reader != null) {
			...
			for (int i = 1; i <= noOfPages; i++) {
				...
				for (int j = 0; j < contentSplitGP.length; j++) {
					...
					if (!(contentSplitCandidate[k].length() == 1 || contentSplitCandidate[k].contains("GRAM PANCHAYAT")
							|| contentSplitCandidate[k].contains("Name of the elected Candidate"))) {
